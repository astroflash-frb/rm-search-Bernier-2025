...
parser.add_argument(
    ...
    help='Frequency bands to process.'
)
parser.add_argument(
    "-i", 
    "--start_file_ind", 
...
# Save data
# ------------------------------------------------------------------------------
# source path is '/scratch/abernier/pulsar_data/2022_CHIME_B2111+46',  
# so source name is 2022_CHIME_B2111+46
# save dir is /scratch/abernier/pulsar_data/baseband_full_stokes
# want to save data to
# SAVE_DIR/{SOURCE_NAME}/{OBS_NIGHT}_bands{FREQ_BANDS as a string}_avg{NSAMPLES_PER_CHUNK}_{NFILES}files.npz

source_name = SOURCE_DIR.parts[-2] + '_' + SOURCE_DIR.parts[-1]
save_filename = f'{OBS_NIGHT}_bands{"".join(map(str,FREQ_BANDS))}_avg{NSAMPLES_PER_CHUNK}_{NFILES}files.npz'
save_path = SAVE_DIR + f'/{source_name}/' + save_filename
Path(SAVE_DIR + f'/{source_name}/').mkdir(parents=True, exist_ok=True)
